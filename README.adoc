This is a fork to provide support/tracking for the West Mountain Radio Epic PWRGate

https://www.westmountainradio.com/product_info.php?products_id=epic-pwrgate

Firmware on device tested is 1.36 https://www.westmountainradio.com/exe/setup_epic_firmware.exe
...
Create a /etc/udev/rules.d/99-nut-ups.rules file and add the following line 

SUBSYSTEM=="tty", ATTRS{idVendor}=="2405", ATTRS{idProduct}=="0013", SYMLINK+="ttyPwrGate", GROUP="nut", MODE="0664", TAG+="systemd"

If you're a Home Assistant User, this is the card I'm using to monitor this via the NUT integration there

cards:
  - type: heading
    heading: Epic PwrGate
  - type: history-graph
    entities:
      - name: Status
        entity: sensor.pwrgate_status
    hours_to_show: 4
  - type: gauge
    entity: sensor.pwrgate_battery_voltage
    name: Battery Charge
    severity:
      green: 12.8
      yellow: 13.4
  ...
  - type: history-graph
    entities:
      - name: Input Voltage
        entity: sensor.pwrgate_input_voltage
